/**
 * PDF Page Counter using PDF.js (Local npm package version)
 * 
 * This version uses the local pdfjs-dist package from node_modules.
 * It requires a bundler like Webpack, Vite, or Rollup to work.
 * 
 * For direct browser usage without a bundler, use counter.js instead (CDN version).
 */
...
if (typeof window !== 'undefined') {
    // Browser environment with bundler
...
export async function getPdfSummary(pdfData) {
    try {
        const info = await getPdfInfo(pdfData);
        const uniformity = await checkPdfUniformity(pdfData);
        
        let summary = {
            pageCount: info.pageCount,
            hasUniformPages: uniformity.isUniform
        };
        
        if (uniformity.isUniform && uniformity.commonSize) {
            const size = uniformity.commonSize;
            ...
            summary.uniqueSizes = uniformity.uniqueSizes.map(size => ({
